...
CREATE TABLE domain.documents (
  ...
  CONSTRAINT documents_media_type_is_pdf
    CHECK (media_type = 'application/pdf'),
  CONSTRAINT documents_byte_size_positive
    CHECK (byte_size > 0),
  CONSTRAINT documents_sha256_lower_hex
    CHECK (sha256 ~ '^[0-9a-f]{64}$'),
  CONSTRAINT documents_page_count_positive
    CHECK (page_count IS NULL OR page_count > 0),
  CONSTRAINT documents_source_kind_remote_only
    CHECK (source_kind = 'remote_execution'),
  CONSTRAINT documents_retention_mode_temporary_only
    CHECK (retention_mode = 'temporary_execution'),
  CONSTRAINT documents_updated_after_created
    CHECK (updated_at >= created_at),
  CONSTRAINT documents_deleted_after_created
    CHECK (deleted_at IS NULL OR deleted_at >= created_at),
  CONSTRAINT documents_owner_workspace_unique
...
-- This is intentionally not unique: the same bytes may be registered in more
-- than one browser workspace or submitted again as a distinct run.
...
CREATE TABLE domain.artifacts (
  ...
  CONSTRAINT artifacts_schema_version_not_blank
    CHECK (
      ...
      OR (schema_version = btrim(schema_version) AND schema_version <> '')
    ),
  CONSTRAINT artifacts_sha256_lower_hex
    CHECK (sha256 ~ '^[0-9a-f]{64}$'),
...
CREATE TABLE domain.remote_artifact_ledger (
  ...
  CONSTRAINT remote_artifact_bucket_valid
    CHECK (
      blob_bucket = btrim(blob_bucket)
      AND blob_bucket <> ''
      AND blob_bucket !~ '[/\\\x00-\x1f\x7f]'
    ),
  CONSTRAINT remote_artifact_key_valid
    CHECK (
      blob_key <> ''
      AND blob_key !~ '^/'
      AND blob_key !~ '/$'
  ...
  CONSTRAINT remote_artifact_ref_unique
    UNIQUE (blob_bucket, blob_key),
  CONSTRAINT remote_artifact_expiry_after_creation
    CHECK (expires_at > created_at),
  CONSTRAINT remote_artifact_imported_after_creation
    CHECK (imported_at IS NULL OR imported_at >= created_at),
  CONSTRAINT remote_artifact_delete_requested_after_creation
    CHECK (delete_requested_at IS NULL OR delete_requested_at >= created_at),
  CONSTRAINT remote_artifact_deleted_after_creation
    CHECK (deleted_at IS NULL OR deleted_at >= created_at),
  CONSTRAINT remote_artifact_cleanup_attempts_nonnegative
    CHECK (cleanup_attempts >= 0),
  CONSTRAINT remote_artifact_deleted_state_consistent
    CHECK ((state = 'deleted') = (deleted_at IS NOT NULL)),
  CONSTRAINT remote_artifact_imported_state_consistent
    CHECK (state <> 'imported' OR imported_at IS NOT NULL),
  CONSTRAINT remote_artifact_delete_pending_state_consistent
    CHECK (state <> 'delete_pending' OR delete_requested_at IS NOT NULL),
  CONSTRAINT remote_artifact_updated_after_created
    CHECK (updated_at >= created_at)
);
